...
namespace UnityEngine.EventSystems
{
    ...
    public class VRInputModule : PointerInputModule
    {
        ...
        public VRLaserPointer m_Cursor;
        ...
        // Allow turning on/off the ability for this script
        // to manipulate the laser. We do this
        // because we want to handle certain UI stuff
        // (like dragging a model into the scene)
        // from elsewhere, so that other code should be in
        // charge of the laser handling
        [SerializeField]
        private bool m_UseLaser = true;
        ...
        public void SetUseLaser(bool active)
        {
            m_UseLaser = active;
        }
        ...
        private bool SendSubmitEventToSelectedObject()
        {
            ...
            // This caused weird issues where the scroll view would not respond
            // correctly after dragging. You would drag, and then click down
            // and it would register as a click (even though it should wait until
            // you click up)
        ...
        private static readonly Vector3[] _workingCorners = new Vector3[4];
        /// <summary>
        /// For RectTransform, calculate it's normal in world space
        /// </summary>
        static Vector3 GetRectTransformNormal(RectTransform rectTransform)
        {
            rectTransform.GetWorldCorners(_workingCorners);
            Vector3 BottomEdge = _workingCorners[3] - _workingCorners[0];
            Vector3 LeftEdge = _workingCorners[1] - _workingCorners[0];
            rectTransform.GetWorldCorners(_workingCorners);
        ...
        virtual protected MouseState GetGazePointerData()
        {
            ...
            if (vrRaycaster)
            {
                ...
                if (graphicRect != null)
                {
                    ...
                    if (m_UseLaser)
                    {
                        Vector3 localPos = vrRaycaster.transform.InverseTransformPoint(raycast.worldPosition);
                        m_Cursor.SetCursorStartDest(rayTransform, localPos, vrRaycaster.transform);
                    }
                }
            }
            ...
            if (physicsRaycaster)
            {
                Debug.Log("Using physics raycaster");
                ...
                if(m_UseLaser)
                    m_Cursor.SetCursorStartDest(rayTransform.position, position);
            }
        ...
        protected MouseState GetCanvasPointerData()
        {
            ...
            if (activeGraphicRaycaster)
            {
                ...
                VRRaycaster ovrRaycaster = raycast.module as VRRaycaster;
        ...
        {
            var pressed = Input.GetKeyDown(gazeClickKey) || ControllerAbstraction.Instances[0].GetMenuInteractDown();
            var released = Input.GetKeyUp(gazeClickKey) || ControllerAbstraction.Instances[0].GetMenuInteractUp();
