...
public class VRRaycaster : GraphicRaycaster, IPointerEnterHandler
{
    ...
    public override Camera eventCamera
    {
        get
        {
            ...
            return Orchestrator.Instance.MainCamera;
        }
    }
    ...
    /// <summary>
    /// Performs a raycast using the pointer object attached to this VRRaycaster
    /// </summary>
    /// <param name="eventData"></param>
    /// <param name="resultAppendList"></param>
    ...
    private static readonly Vector3[] workingCorners = new Vector3[4];
    /// <summary>
    ...
    {
        rectTransform.GetWorldCorners(workingCorners);
        Plane plane = new Plane(workingCorners[0], workingCorners[1], workingCorners[2]);
        ...
        Vector3 BottomEdge = workingCorners[3] - workingCorners[0];
        Vector3 LeftEdge = workingCorners[1] - workingCorners[0];
        float BottomDot = Vector3.Dot(intersection - workingCorners[0], BottomEdge);
        float LeftDot = Vector3.Dot(intersection - workingCorners[0], LeftEdge);
        if (BottomDot < BottomEdge.sqrMagnitude && // Can use sqrMag because BottomEdge is not normalized
            LeftDot < LeftEdge.sqrMagnitude &&
                BottomDot >= 0 &&
                LeftDot >= 0)
        {
            worldPos = workingCorners[0] + LeftDot * LeftEdge / LeftEdge.sqrMagnitude + BottomDot * BottomEdge / BottomEdge.sqrMagnitude;
            return true;
        }
        else
        {
