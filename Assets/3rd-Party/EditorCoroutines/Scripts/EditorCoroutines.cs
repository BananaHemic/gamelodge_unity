...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		struct YieldWebRequestAsyncOperation : ICoroutineYield
		{
			public UnityWebRequestAsyncOperation WebReq;

			public bool IsDone(float deltaTime)
			{
                return WebReq.isDone;
			}
		}
		...
		static bool Process(EditorCoroutine coroutine)
		{
            ...
            //Debug.Log("coroutine type: " + (current != null ? current.GetType().ToString() : "null"));
			if (current == null)
			{
				return false;
			}
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            {
                coroutine.currentYield = new YieldWebRequestAsyncOperation { WebReq = (UnityWebRequestAsyncOperation)current };
            }
			else
			{
				Debug.LogException(
